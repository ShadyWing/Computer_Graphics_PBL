...
struct my_triangle_3DModel
{
	...
	float reflection = 0.001;
	//以下用于计算直接光照能量
	float material_ambient_rgb_reflection[3] = { 0.5,0.5,0.5 }; //环境光反射系数，初始不反射
	float material_diffuse_rgb_reflection[3] = { 0.5,0.5,0.5 };//漫反射系数，初始不反射
	float material_specular_rgb_reflection[3] = { 0.5,0.5,0.5 };//镜面光反射系数，初始不反射
